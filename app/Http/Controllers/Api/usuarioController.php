...
class usuarioController extends Controller
{
    public function login(Request $request)
    {
        ...
        $usuario = Usuario::where('correo_electronico', $credenciales['correo_electronico'])->first();

        if ($usuario && Hash::check($credenciales['contrasena'], $usuario->contrasena)) {
            $token = $usuario->createToken('API Usuario Token')->plainTextToken;
    ...
    public function store(Request $request){
        $validator = Validator::make($request->all(), [
            ...
            "correo_electronico" => "required|email|max:255|unique:usuario",
